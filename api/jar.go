...
func initializeJars(ctx context.Context) error {
	...
	err := createIndexes(ctx, coll,
		[]mongo.IndexModel{
			...
			newSimpleIndex("pcRun", "pcRun", false, true, false), // TODO: required, but old values may not have it... See if we can fix this... Maybe do a migration for it?
...
func importJarHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		newFileNameWithPrefixPath, errr := pics.SavePicFile(r.Context(), fieldBytes, "bag", string(b58id), "img")
		if errr != nil {
			err = errr
