...
var (
	// Fluid Types
	fluids = []Fluid{` + fluidsTpl + `}
{{range $name, $value := .FluidTypes}}` + fluidVarTpl + `
{{end}}
	// TODO: add grains, sugars, nutrients, colorants, colors, additives, antibiotics
)
...
const constCollNameTpl = `	{{$name}} = "{{$value}}"`
const fluidVarTpl = `	{{$info.CollConstName}} = Fluid("{{$info.CollName}}")`
const fluidsTpl = `{{- $isFirst := true }}
{{- range $name, $value := .FluidTypes }}
  {{- if not $isFirst }}, {{ end }}
  {{- $key }}: {{ $val }}
  {{- $isFirst = false }}
{{- end }}}`
